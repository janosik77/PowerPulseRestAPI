...
public class ServiceOrderConfiguration : IEntityTypeConfiguration<ServiceOrder>
{
    public void Configure(EntityTypeBuilder<ServiceOrder> b)
    {
        ...
        b.Property(x => x.AppointmentAt).HasColumnName("appointment_at");

        b.Property(x => x.AppointmentNote)
        ...
        b.Property(x => x.SentToServiceAt).HasColumnName("sent_to_service_at");
        b.Property(x => x.ReturnedFromServiceAt).HasColumnName("returned_from_service_at");

        b.Property(x => x.MileageAtService).HasColumnName("mileage_at_service");
        ...
        b.HasIndex(x => x.AppointmentAt);

        // Constrainty
        b.HasCheckConstraint("ck_service_order_total_cost_nonneg", "total_cost IS NULL OR total_cost >= 0");
        b.HasCheckConstraint("ck_service_order_mileage_nonneg", "mileage_at_service IS NULL OR mileage_at_service >= 0");

        // approval spójność
        b.HasCheckConstraint(
            "ck_service_order_approval_consistency",
            "(approved_at IS NULL AND approved_by_user_id IS NULL) OR (approved_at IS NOT NULL AND approved_by_user_id IS NOT NULL)"
        );

        // completion spójność
        b.HasCheckConstraint(
            "ck_service_order_completion_consistency",
            "(completed_at IS NULL AND completed_by_user_id IS NULL) OR (completed_at IS NOT NULL AND completed_by_user_id IS NOT NULL)"
        );

        // daty serwisu
        b.HasCheckConstraint(
            "ck_service_order_service_dates",
